...
public class JsonProcessingHelperTest {
        ...
        public void getJavaTypeFromJsonValueTest() {
                ...
                Assertions.assertEquals(null, JsonProcessingHelper
                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));

                jsonObject = jsonObjectBuilder.add(key, "123").build();
                Assertions.assertEquals("123", JsonProcessingHelper
                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));

                jsonObject = jsonObjectBuilder.add(key, new Integer(123)).build();
                Assertions.assertEquals(true,
                                jsonObject.get(key).getValueType() == ValueType.NUMBER);
                Assertions.assertEquals(true, jsonObject.getJsonNumber(key).isIntegral());

                jsonObject = jsonObjectBuilder.add(key, 2.4).build();
                Assertions.assertEquals(2.4, JsonProcessingHelper
                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));

                jsonObject = jsonObjectBuilder.add(key, true).build();
                Assertions.assertEquals(true, JsonProcessingHelper
                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));

                jsonObject = jsonObjectBuilder.add(key, false).build();
                Assertions.assertEquals(false, JsonProcessingHelper
                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));

                jsonObject = jsonObjectBuilder.add(key, new Long(13)).build();
                Assertions.assertEquals(true, ((JsonNumber) jsonObject.get(key)).isIntegral());
                Assertions.assertEquals(true, 13 == (long) JsonProcessingHelper
                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));
                Assertions.assertEquals(13L, JsonProcessingHelper
                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));
        }
        ...
        public void getJavaListTypeFromJsonValueTest() {
                ...
                JsonObject jsonObject = jsonObjectBuilder.add(key,
                                jsonArrayBuilder.add("test").add("test2").addNull().build())
                                .build();
                ...
                // Only two elements, null causes no exception and is not added
        ...
        public void testUnsupportedOperationException() {
                ...
                JsonObject jsonObject = jsonObjectBuilder
                                .add(key, jsonObjectBuilder.add("key", "value").build()).build();
                Assertions.assertThrows(UnsupportedOperationException.class,
                                () -> JsonProcessingHelper
                                                .getJavaTypeFromJsonJsonValue(jsonObject.get(key)));
        }
}
